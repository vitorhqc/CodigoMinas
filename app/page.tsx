...
export default function Home() {
  ...
  return (
    <div className="flex w-full h-screen flex-wrap justify-items-center items-center justify-center content-center ">
      <main className="flex flex-col h-full gap-[32px] row-start-2 justify-items-center items-center justify-center content-center">
        ...
        <button className={`w-40 text-center bg-stone-400 rounded-md text-white font-normal hover:font-bold ${!codigook ? 'mb-52' : ''}`} onClick={() => buscarCliente(sigla, cpfcnpj)}>Gerar código</button>
        {codigook && <input className={`w-50 text-center rounded-md border-1 border-black bg-red-200 ${codigook ? 'mb-52' : ''}`} value={codigo} readOnly></input>}
  ...
  async function buscarCliente(sigla: string = '', CPFCNPJ: string = '') {
    try{
      ...
      if (!res.ok){
        alert('Estes dados nao foram encontrados!! Confira a SIGLA e CPF/CNPJ e tente novamente!');
        setCodigook(false);
        return;
      }
      setCodigook(true);
